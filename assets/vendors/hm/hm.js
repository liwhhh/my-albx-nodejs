(function () {
    ...
    /*
        jquery的parents方法的功能
            获取元素的祖宗十八代
        两种用法
            1. $(选择器).parents(); 得到祖宗十八代
            2. $(选择器).parents(祖宗的条件); 得到满足条件的祖宗十八代
        思路：
            这是一个实例方法，需要把实例方法放在原型上
    */
    // 封装一个递归的获取祖宗十八代的方法
    function f1(e) {
        let parents = [];
    ...
    Init.prototype.parents = function (selector) {
        ...
        for (let i = 0; i < this.length; i++) {
            let parents = f1(this[i]);
            arr = arr.concat(parents);
        }
        if (selector === undefined) {
            // 返回 - 的应该是一个伪数组 - 是Init的实例对象
            let _this = new Init();
            ...
            // 传入了一个筛选前代元素的条件
            // 根据选择器 先获取页面上所有的满足条件的元素
            let temp = document.querySelectorAll(selector);
            // 如果元素既在 arr 里面， 又在根据选择器获取的数组里面，同时满足两个条件
            let tp = [];
            // 判断是否同时是 元素的前代元素，又是根据选择器获取的元素
            temp.forEach(e=>{
                if(arr.indexOf(e) != -1){
                    tp.push(e);
                }
